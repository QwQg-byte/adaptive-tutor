...
## 开源数据说明
  ...
  本仓库开源 AdaptiveTutor 的 Agent 编排、知识图谱检索、自适应学习路径规划、学习状态管理、前后端应用及测试代码。

  完整教学题库、知识图谱数据及部分多媒体素材因第三方授权和再分发边界尚待确认，暂不随开源仓库发布。本仓库后续将补充可公开分发的示例数据，用于复现知识检索、路径规划和伴学 Agent 任务
  闭环。

  运行完整 Demo 需要自行准备符合 `graph/data/processed/` 数据格式的知识图谱数据，并配置 Neo4j、LLM API 及可选的语音服务。
